...
def show_results_t(results, label_values=None, agregated=False):
    text = ""
...
def sample_gt(gt, train_size, mode='random'):
    ...
    if mode == 'random':
        # print("训练集的占比: ", train_size)
        train_indices, test_indices = sklearn.model_selection.train_test_split(X, train_size=train_size, stratify=y)
        # train_indices = [list(t) for t in zip(*train_indices)]
        # a = len(train_indices[1])
        # test_indices = [list(t) for t in zip(*test_indices)]
        # a = gt[train_indices]
        # b = np.count_nonzero(a)
        for i in train_indices:
            train_gt[i[0]][i[1]] = gt[i[0]][i[1]]
        # print("训练标签的非0个数", np.count_nonzero(train_gt))
        for j in test_indices:
            test_gt[j[0]][j[1]] = gt[j[0]][j[1]]
        # b = np.count_nonzero(test_gt)
    elif mode == 'fixed':
