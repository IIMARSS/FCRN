...
from utils import metrics, convert_to_color_, convert_from_color_, \
    ...
    sample_gt, build_dataset, show_results, compute_imf_weights, get_device, show_results_t
from datasets import get_dataset, HyperX, open_file, DATASETS_CONFIG
from models import get_model, train, test, save_model, train_student_net, train_teacher_net
...
parser.add_argument('--dataset', type=str, default="xianchang1", choices=dataset_names,
                    help="Dataset to use.")
...
if MODEL == 'hybridsn_ps25' or MODEL == 'ssfttnet' or MODEL == 'DSFormer':
    a_shape = img.shape
...
for run in range(N_RUNS):
    ...
    if MODEL == 'SVM_grid':
        ...
        class_weight = 'balanced' if CLASS_BALANCING else None
        clf = sklearn.linear_model.SGDClassifier(class_weight=class_weight, learning_rate='optimal', tol=1e-2,
                                                 average=10)
        clf.fit(X_train, y_train)
        save_model(clf, MODEL, DATASET)
        ...
        clf = sklearn.model_selection.GridSearchCV(clf, {'n_neighbors': [1, 3, 5, 10]}, verbose=4, n_jobs=4)
        clf.fit(X_train, y_train)
        clf.fit(X_train, y_train)
        save_model(clf, MODEL, DATASET)
        prediction = clf.predict(img.reshape(-1, N_BANDS))
        ...
        # Neural network
        if MODEL == 'kd':
            model_t, model_s, optimizer, loss, hyperparams = get_model(MODEL, **hyperparams)
        else:
        ...
        if MODEL == 'kd':
            # 训练教师网络
            # train_teacher_net(model_t, train_loader, hyperparams['epoch'], lr=hyperparams['learning_rate'],
            #                   weights=hyperparams['weights'], scheduler=hyperparams['scheduler'],
            #                   device=hyperparams['device'], supervision=hyperparams['supervision'],
            #                   val_loader=val_loader, display=viz, model_name=MODEL)
            # probabilities_t = test(model_t, img, hyperparams)
            # prediction_t = np.argmax(probabilities_t, axis=-1)
            # run_results_t = metrics(prediction_t, test_gt, ignored_labels=hyperparams['ignored_labels'],
            #                         n_classes=N_CLASSES)
            # show_results_t(run_results_t, label_values=LABEL_VALUES)
            # 训练学生网络
            print("Network :")
            with torch.no_grad():
                ...
                summary(model_s.to(hyperparams['device']), input.size()[1:])
            train_student_net(model_t, model_s, optimizer, loss, train_loader, hyperparams['epoch'],
            ...
            probabilities = test(model_s, img, hyperparams)
