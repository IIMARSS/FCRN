...
def get_model(name, **kwargs):
    ...
    if name == 'SSPN':
        ...
        patch_size = kwargs.setdefault('patch_size', 9)
        center_pixel = False
        model_t = SSPN(patch_size=patch_size, classes=n_classes, channels=n_bands)
        model_s = light_cnn(patch_size=patch_size, bands=n_bands, classes=n_classes)
        # model_s = ManualTransformer(patch_size=patch_size, bands=n_bands, num_classes=n_classes)
        lr = kwargs.setdefault('learning_rate', 0.00008)
        # optimizer = optim.SGD(model.parameters(), lr=0.01, weight_decay=0.0005)
        optimizer = optim.RMSprop(model_s.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
        kwargs.setdefault('epoch', 200)
        kwargs.setdefault('batch_size', 16)
    elif name == 'light_cnn':
        patch_size = kwargs.setdefault('patch_size', 9)
        center_pixel = False
        model = light_cnn(patch_size=patch_size, bands=n_bands, classes=n_classes)
        # model_s = ManualTransformer(patch_size=patch_size, bands=n_bands, num_classes=n_classes)
        lr = kwargs.setdefault('learning_rate', 0.00008)
        # optimizer = optim.SGD(model.parameters(), lr=0.01, weight_decay=0.0005)
        optimizer = optim.RMSprop(model.parameters(), lr=lr)
    ...
    if name == 'kd':
        return model_t, model_s, optimizer, criterion, kwargs
    else:
...
class bottom_up_333(nn.Module):
    ...
    def forward(self, x):
        x = self.se(x)
        p1 = self.conv1(x) # (129, 5, 5)
        p2 = self.conv2(p1) # (65, 3, 3)
        p3 = self.conv3(p2) # (33, 2, 2)
        p4 = self.conv4(p3) # (17, 1, 1)
        p5 = self.conv5(p4) # (9, 1, 1)
...
class bottom_up_335(nn.Module):
    ...
    def forward(self, x):
        x = self.se(x)
        p1 = self.conv1(x) # (128, 5, 5)
        p2 = self.conv2(p1) # (64, 3, 3)
        p3 = self.conv3(p2) # (32, 2, 2)
        p4 = self.conv4(p3) # (16, 2, 2)
        p5 = self.conv5(p4) # (8, 2, 2)
...
class bottom_up_337(nn.Module):
    ...
    def forward(self, x):
        x = self.se(x)
        p1 = self.conv1(x) # (127, 5, 5)
        p2 = self.conv2(p1) # (62, 3, 3)
        p3 = self.conv3(p2) # (30, 2, 2)
        p4 = self.conv4(p3) # (14, 2, 2)
        p5 = self.conv5(p4) # (7, 3, 3)
...
class light_cnn(nn.Module):
    def __init__(self, patch_size, bands, classes):
        super(light_cnn, self).__init__()
        self.patch_size = patch_size
        self.bands = bands

        self.conv1 = nn.Sequential(
            nn.Conv2d(bands, 16, kernel_size=3, padding=1),
            nn.BatchNorm2d(16),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, padding=1)
        )
        self.conv2 = nn.Sequential(
            nn.Conv2d(16, classes, kernel_size=3, padding=1),
        ...
        self.out = nn.Linear(self.feature_size, classes)

    def _feature_size(self):
        with torch.no_grad():
            x = torch.zeros((1, self.bands, self.patch_size, self.patch_size))
            x = self.conv1(x)
            x = self.conv2(x)
            # x = self.conv3(x)
            _, a, b, c = x.size()
...
def train(net, optimizer, criterion, data_loader, epoch, model_name, scheduler=None,
    ...
    val_accuracies = []
    # print("data 的个数", len(data_loader))
    ...
    for e in tqdm(range(1, epoch + 1), desc="Training the network"):
        ...
        for batch_idx, (data, target) in tqdm(enumerate(data_loader), total=len(data_loader)):
            ...
            # print("data的大小：", data.shape)


            # print("data 的个数", len(data))
        ...
        if val_loader is not None:
            val_acc = val(net, val_loader, t_s='s', model_name=model_name, device=device, supervision=supervision)
...
# 知识蒸馏
# 训练教师网络
def train_teacher_net(net, data_loader, epoch, lr, weights, model_name, scheduler=None,
                      display_iter=100, device=torch.device('cpu'), display=None,
                      val_loader=None, supervision='full'):
    ...
    for e in tqdm(range(1, epoch + 1), desc="Training the teacher network"):
        # Set the network to training mode
        net.train()
        avg_loss = 0.
...
# 训练学生网络
def train_student_net(net_t, net_s, optimizer, criterion, data_loader, epoch, model_name, scheduler=None,
                      display_iter=100, device=torch.device('cpu'), display=None,
                      val_loader=None, supervision='full'):
    if criterion is None:
        raise Exception("Missing criterion. You must specify a loss function.")
    ...
    # 直接导入对应权重
    # net_t.load_state_dict(torch.load('./checkpoints/spn/xianchang2/run_epoch200_1.00.pth'))
    net_t.load_state_dict(torch.load('./checkpoints/spn/xianchang1/run_epoch200_1.00.pth'))
    ...
    for e in tqdm(range(1, epoch + 1), desc="Training the student network"):
        # Set the network to training mode
        net_t.eval()
        net_s.train()
        avg_loss = 0.
        ...
        for batch_idx, (data, target) in tqdm(enumerate(data_loader), total=len(data_loader)):
            ...
            optimizer.zero_grad()
            with torch.no_grad():
                _, _, _, output_t = net_t(data)


            output_s = net_s(data)
            loss_s = criterion(output_s, target)

            soft_t = nn.Softmax(dim=1)(output_t / T)
            soft_s = nn.LogSoftmax(dim=1)(output_s / T)
            loss_kd = nn.KLDivLoss(reduction='batchmean')(soft_s, soft_t) * (T ** 2)
            loss = loss_s * alpha + loss_kd * (1 - alpha)
            ...
            del (data, target, loss, output_s)

        # Update the scheduler
        avg_loss /= len(data_loader)
        if val_loader is not None:
            val_acc = val(net_s, val_loader, t_s='s', model_name=model_name, device=device, supervision=supervision)
            val_accuracies.append(val_acc)
            metric = -val_acc
        else:
            metric = avg_loss
        ...
        # Save the weights
        if e % save_epoch == 0:
            save_model(net_s, camel_to_snake(str(net_s.__class__.__name__)), data_loader.dataset.name, epoch=e, metric=abs(metric))
...
def val(net, data_loader, model_name, t_s='t', device='cpu', supervision='full'):
